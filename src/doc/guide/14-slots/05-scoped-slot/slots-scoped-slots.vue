<template>
  <div>
    ...
    <small>
      <ul>
        <li>
          组件元素之间的内容默认是属于父组件作用域的， 只能访问父组件的数据；
          那如何才能访问子组件的数据呢 ?
          <ol>
            <li>
              在子组件的 slot
              元素上，将要暴露给父组件的数据绑定到一个属性上（例如：data）；
            </li>
            <li>
              在父组件中，组件元素之间的元素上， 通过 slot-scope
              属性拿到子组件暴露出来的作用域对象（scope）；
            </li>
            <li>通过 scope.data 就可以拿到子组件暴露出来的数据；</li>
          </ol>
        </li>
      </ul>
    </small>
    ...
    <small>
      <ul>
        <li>slot-scope 属性的值可以使用'函数参数结构'的语法；</li>
      </ul>
    </small>
    <TodoList :todos="todos">
