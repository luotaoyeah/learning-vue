<template>
  <div>
    <ul>
      <li>
        要想通过 createElement() 方法实现与模板相同的效果， 需要了解 createElement() 方法的参数；
      </li>
      <li>
        ...
        <ul>
          <li>
            ...
            <ul>
              ...
              <li>
                如果是 Function， 表示一个异步的函数， 返回'HTML 标签名称'或者'组件选项对象'；
              </li>
            </ul>
          </li>
          ...
          <li>
            ...
            <ul>
              ...
              <li>
                如果是 Array， 则其中每一个元素又是 createElement() 方法调用返回的结果；
              </li>
            </ul>
          </li>
          <li>
            第二个参数是可选的， 那么我们可以通过实际第二个参数的类型， 来判断是否省略了第二个参数；
            <ul>
              <li>
    ...
    <ul>
      <li>
        在模板中， v-bind:class 和 v-bind:style 会被特殊对待； 在属性对象中， 它们也分别有对应的顶层属性；
      </li>
      <li>
        属性对象中， 可以绑定普通的 HTML 属性， 也可以绑定 DOM 属性， 还可以绑定组件的 props；
    ...
    <ul>
      <li>
        组件中所有的 VNode 必须是唯一的， 不能重复， 即不能将同一个 VNode 用在多个地方；
      </li>
      <li>
        如果需要重复使用某个 VNode， 可以使用工厂方法， 每次调用工厂方法就返回一个 VNode 的实例；
      </li>
    </ul>
  </div>
</template>
