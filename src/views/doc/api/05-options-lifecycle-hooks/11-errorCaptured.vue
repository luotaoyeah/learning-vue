  ...
  <div>
    <ul>
      <li>
        ...
        <ul>
          <li>
            如果定义了 Vue.config.errorHandler() 函数， 错误会传递到该函数；
          </li>
          <li>
            如果组件继承链上有多个 errorCaptured() 函数， 它们会从下到上依次触发；
          </li>
          <li>
            如果在 errorCaptured() 里面抛出了其他的错误， 该错误会和捕获的错误一起传递到 Vue.config.errorHandler()；
          </li>
          <li>
            如果在 errorCaptured() 返回了 false， 则错误不会继续往上面的 errorCaptured() 传递， 也不会往
            Vue.config.errorHandler() 传递；
          </li>
        </ul>
      </li>
    </ul>
